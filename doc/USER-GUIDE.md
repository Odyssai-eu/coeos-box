...
| Terme | Ce que c'est |
|---|---|
| **Axe de compétence** | Une catégorie *benchmarkée*. 16 axes livrés : `creative`, `redac_pro`, `legal_rgpd`, `legal_complex`, `reasoning`, `calc`, `python`, `code_general`, `debug`, `react`, `swift`, `refactoring`, `plan_decompo`, `plan_spec`, `plan_judgment`, `fast_tools`. |
