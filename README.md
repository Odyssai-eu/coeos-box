...
**Theseus** ; le client est **Nemo**, [Odyssai-eu/coeos](https://github.com/Odyssai-eu/coeos),
...
```
 ...
 (Claude Code, Aider, Cline,      │ classe la requête        └▶  OdyssAI-X (local, sans clé)
  Continue, SDK OpenAI/Anthropic) │ sur un axe, relaie
                                   └── console Theseus (:4600/dashboard)
```
...
Console **Theseus** sur `http://<hôte>:4600/dashboard` (aussi servie sur `:4800`).
...
```bash
curl -X PUT http://localhost:4600/admin/platform/providers/odyssai \
...
ici comme socle de la box. Console : **Theseus**.
